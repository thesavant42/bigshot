    ...
    def test_enumeration_service_invalid_source(self, client):
        ...
        with pytest.raises(ValueError, match="Unsupported sources"):
            service.start_enumeration(
                domains=["example.com"], sources=["invalid_source"], options={}
            )
...
class TestJobAPI:
    ...
    def test_data_normalization_endpoint(self, client, auth_headers):
        ...
        with patch(
            "app.services.job_manager.JobManager.start_data_normalization"
        ) as mock_start:
            mock_job = MagicMock()
    ...
    def test_data_deduplication_endpoint(self, client, auth_headers):
        ...
        with patch(
            "app.services.job_manager.JobManager.start_data_deduplication"
        ) as mock_start:
            mock_job = MagicMock()
            mock_job.to_dict.return_value = {
                "id": 1,
            ...
            response = client.post(
                "/api/v1/jobs/data/deduplicate", headers=auth_headers
            )
    ...
    def test_data_cleanup_endpoint(self, client, auth_headers):
        ...
        with patch(
            "app.services.job_manager.JobManager.start_data_cleanup"
        ) as mock_start:
            mock_job = MagicMock()
            mock_job.to_dict.return_value = {
                "id": 1,
            ...
            response = client.post(
                "/api/v1/jobs/data/cleanup", json={"days_old": 60}, headers=auth_headers
            )
    ...
    def test_job_task_status_endpoint(self, client, auth_headers):
        ...
        with patch("celery_app.celery_app.AsyncResult") as mock_result:
            ...
            response = client.get(
                f"/api/v1/jobs/{job.id}/task-status", headers=auth_headers
            )
...
class TestTaskFunctions:
    ...
    def test_normalize_domain_function(self):
        ...
        for input_domain, expected in test_cases:
            ...
            assert (
                result == expected
            ), f"Failed for {input_domain}: expected {expected}, got {result}"
