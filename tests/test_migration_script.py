...
class TestErrorMessageColumnMigration:
    ...
    def test_migration_script_adds_missing_column(self):
        ...
        try:
            ...
            assert 'error_message' not in column_names, "error_message should not exist initially"
            conn.close()
            
            # Run the migration script
            script_path = Path(__file__).parent.parent / 'scripts' / 'add_error_message_column.py'
            env = os.environ.copy()
            ...
            assert 'error_message' in column_names, f"error_message column should exist after migration. Found: {column_names}"
            conn.close()
    ...
    def test_migration_script_handles_existing_column(self):
        ...
        try:
            ...
            columns = cursor.fetchall()
            column_names = [col[1] for col in columns]
            assert 'error_message' in column_names, "error_message should exist initially"
            conn.close()
            ...
            assert 'error_message' in column_names, f"error_message column should still exist. Found: {column_names}"
            conn.close()
    ...
    def test_migration_script_file_exists(self):
        ...
        if os.name != 'nt':  # Not Windows
            assert os.access(script_path, os.X_OK), f"Migration script is not executable: {script_path}"
    ...
    def test_migration_script_database_connection_error_handling(self):
        ...
        assert result.returncode != 0, "Migration script should fail with invalid database URL"
        output = result.stdout + result.stderr
