...
def test_audit_log_user_id_is_integer(client, auth_headers, app):
    ...
    with app.app_context():
        ...
        for log in audit_logs:
            assert isinstance(log.user_id, int), f"user_id should be integer, got {type(log.user_id)}"
            assert log.user_id == admin_user.id, f"user_id should be {admin_user.id}, got {log.user_id}"
            assert log.action in ["created", "activated", "tested"], f"Unexpected action: {log.action}"
