...
class TestUndefinedVariableFix:
    ...
    def test_connection_timeout_validation_and_assignment(self, client, auth_headers):
        ...
        with client.application.app_context():
            provider = LLMProviderConfig.query.filter_by(name="Test Provider Timeout").first()
    ...
    def test_temperature_validation_and_assignment(self, client, auth_headers):
        ...
        # Verify in database
        with client.application.app_context():
            provider = LLMProviderConfig.query.filter_by(name="Test Provider Temperature").first()
            assert provider is not None
    ...
    def test_default_values_when_not_provided(self, client, auth_headers):
        ...
        # Verify in database
        with client.application.app_context():
            provider = LLMProviderConfig.query.filter_by(name="Test Provider Defaults").first()
            assert provider is not None
    ...
    def test_numeric_inputs_are_handled_correctly(self, client, auth_headers):
        ...
        # Verify in database
        with client.application.app_context():
            provider = LLMProviderConfig.query.filter_by(name="Test Provider Numeric").first()
            assert provider is not None
