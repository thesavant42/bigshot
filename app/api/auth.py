    ...
    try:
        ...
        proof_data = {
            ...
            "environment": {
                ...
                "container_id": os.getenv('CONTAINER_ID', 'local')[:12] if os.getenv('CONTAINER_ID') else 'local'
            },
            "client": {
        ...
        try:
            ...
            db_info = {
                ...
                "connection_url": _mask_credentials(os.getenv('DATABASE_URL', 'Not configured'))
            }
        ...
        try:
            ...
            redis_info = {
                ...
                "connection_url": _mask_credentials(os.getenv('REDIS_URL', 'Not configured'))
            }
        ...
        try:
            ...
            celery_info = {
                ...
                "message": "Celery workers responding" if active_tasks is not None else "No active Celery workers",
                "active_workers": len(active_tasks) if active_tasks else 0,
                "broker_url": _mask_credentials(celery_app.conf.broker_url)
            }
            auth_logger.debug(f"Celery connectivity check: {'SUCCESS' if active_tasks is not None else 'DEGRADED'}")
        except Exception as e:
            celery_info = {
                "status": "FAILED",
                "message": f"Celery connection failed: {str(e)}",
                "broker_url": _mask_credentials(os.getenv('CELERY_BROKER_URL', 'Not configured'))
            }
        ...
        healthy_services = [k for k, v in all_services.items() if v["status"] == "HEALTHY"]
        proof_data["overall_status"] = {
            "healthy_services": len(healthy_services),
            "total_services": len(all_services),
            "status": "HEALTHY" if len(healthy_services) == len(all_services) else "DEGRADED"
        }
        
        auth_logger.info(f"Connectivity proof generated for {current_username}: {len(healthy_services)}/{len(all_services)} services healthy")
