...
def create_debug_package():
    ...
    try:
        debug_log(
            "Debug package export requested", zone="export", user=request.remote_addr
        )
...
def download_debug_package():
    ...
    try:
        ...
        debug_log(
            "Debug package download requested",
            zone="export",
            package_path=str(latest_package),
        )

        return send_file(
            latest_package, as_attachment=True, download_name=latest_package.name
        )
...
def validate_environment():
    ...
    try:
        debug_log(
            "Environment validation requested", zone="env", user=request.remote_addr
        )
...
def validate_filesystem():
    ...
    try:
        debug_log(
            "Filesystem validation requested", zone="docker", user=request.remote_addr
        )

        # Run filesystem validation
        log_filesystem_validation()

        return success_response({"message": "Filesystem validation completed"})
...
def check_docker_context():
    ...
    try:
        debug_log(
            "Docker context check requested", zone="docker", user=request.remote_addr
        )
        ...
        docker_info = {
            ...
            or bool(os.getenv("CONTAINER_ID")),
        ...
        return success_response(
            {"message": "Docker context check completed", "docker_info": docker_info}
        )
...
def log_debug_message():
    ...
    try:
        ...
        debug_log(message, zone=zone, user=request.remote_addr, source="api_request")

        return success_response({"message": f"Debug message logged to zone: {zone}"})
