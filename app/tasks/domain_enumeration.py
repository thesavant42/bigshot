...
def enumerate_domains_task(self, job_id, domains, sources, options):
    ...
    try:
        ...
        from app.tasks.notifications import broadcast_job_update_task, send_job_notification_task
        broadcast_job_update_task.delay(job_id, 'completed', {
            'total_found': len(all_results),
            'new_domains': len([r for r in all_results if not r.get('existing')]),
            'updated_domains': len([r for r in all_results if r.get('existing')])
        })
        ...
        # Broadcast failure
        from app.tasks.notifications import broadcast_job_update_task, send_job_notification_task
        broadcast_job_update_task.delay(job_id, 'failed', {
            'error_message': str(e)
        })
