...
class JobManager:
    ...
    def start_data_normalization(self):
        ...
        # Create job record
        job = Job(type="data_normalization", status="pending", progress=0)
    ...
    def start_data_deduplication(self):
        ...
        # Create job record
        job = Job(type="data_deduplication", status="pending", progress=0)
    ...
    def start_data_cleanup(self, days_old=30):
        ...
        # Create job record
        job = Job(type="data_cleanup", status="pending", progress=0)
    ...
    def _estimate_completion(self, job):
        ...
        if job.progress > 0:
            ...
            if remaining > 0:
                estimated_completion = datetime.now(timezone.utc).timestamp() + remaining
    ...
    def get_job_statistics(self):
        ...
        # Jobs by status
        status_counts = (
            db.session.query(Job.status, func.count(Job.id).label("count"))
            .group_by(Job.status)
            .all()
        )
        ...
        # Jobs by type
        type_counts = (
            db.session.query(Job.type, func.count(Job.id).label("count"))
            .group_by(Job.type)
            .all()
        )
